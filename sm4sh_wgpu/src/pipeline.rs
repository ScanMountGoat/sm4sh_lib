...
pub struct PipelineKey {
    primitive_type: sm4sh_model::PrimitiveType,
...
struct MaterialKey {
    shader_id: u32,
    cull_mode: sm4sh_model::CullMode,
    alpha_test_ref: u16,
...
static PIPELINES: LazyLock<Mutex<HashMap<PipelineKey, wgpu::RenderPipeline>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));

impl PipelineKey {
    pub fn new(mesh: &NudMesh) -> Self {
        Self {
            primitive_type: mesh.primitive_type,
            material: mesh.material1.as_ref().map(|m| MaterialKey {
...
pub fn model_pipeline(
    ...
    let topology = match key.primitive_type {
    ...
    let blend = key.material.as_ref().map(blend_state);

    let strip_index_format = topology.is_strip().then_some(wgpu::IndexFormat::Uint16);

    let cull_mode = key.material.as_ref().and_then(|m| match m.cull_mode {
    ...
    // Only create unique pipelines once to greatly reduce loading times.
    // Pipelines are cheap to clone, which avoids needing to lock the static cache for rendering.
    PIPELINES
        .lock()
        .unwrap()
        .entry(*key)
        .or_insert_with(|| {
            let program = key
                .material
                .and_then(|m| shared_data.database.get_shader(m.shader_id));
            let alpha_test_ref_func = key
                .material
                .as_ref()
            ...
            let label = key.material.map(|m| format!("{:X}", m.shader_id));
...
fn blend_state(m: &MaterialKey) -> wgpu::BlendState {
    wgpu::BlendState {
